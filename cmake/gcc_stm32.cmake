...
SET(STM32_SUPPORTED_FAMILIES F0 F1 F2 F3 F4 F7 L0 L1 L4 CACHE INTERNAL "stm32 supported families")
...
SET(CMAKE_AR ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-gcc-ar${TOOL_EXECUTABLE_SUFFIX} CACHE INTERNAL "ar tool")
SET(CMAKE_RANLIB ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-gcc-ranlib${TOOL_EXECUTABLE_SUFFIX} CACHE INTERNAL "ranlib tool")
SET(CMAKE_NM ${TOOLCHAIN_BIN_DIR}/${TARGET_TRIPLET}-gcc-nm${TOOL_EXECUTABLE_SUFFIX} CACHE INTERNAL "nm tool")
...
INCLUDE(dev_stm32${STM32_FAMILY_LOWER})

MACRO(STM32_GET_CHIP_TYPE CHIP CHIP_TYPE)
    STRING(SUBSTRING ${CHIP} 0 11 STM32_CODE)
    STRING(REGEX MATCH "^STM32[FLW][0-47][0-9][0-9][A-Z][0-9A-Z]" CHIP_PREFIX ${CHIP})
    STRING(REGEX MATCH "-([AX])$" CHIP_SUFFIX ${CHIP})
    IF(CHIP_SUFFIX STREQUAL "")
        SET(STM32_CODE ${CHIP_PREFIX})
    ELSE()
        SET(STM32_CODE ${CHIP_PREFIX}_${CMAKE_MATCH_1})
    ENDIF()
    SET(${CHIP_TYPE} ${STM32_CODE})
ENDMACRO()

MACRO(STM32_GET_CHIP_PARAMETERS CHIP FLASH_ORIGIN FLASH_SIZE RAM_ORIGIN RAM_SIZE CCRAM_ORIGIN CCRAM_SIZE)
    STM32_GET_CHIP_TYPE(${CHIP} TYPE)
    IF(DEV_${TYPE})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_FLASH_START} ${FLASH_ORIGIN})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_FLASH_SIZE} ${FLASH_SIZE})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_RAM_START} ${RAM_ORIGIN})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_RAM_SIZE} ${RAM_SIZE})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_CCRAM_START} ${CCRAM_ORIGIN})
        LIST(GET DEV_${TYPE} ${DEV_TABLE_CCRAM_SIZE} ${CCRAM_SIZE})
    ELSE()
        IF(NOT ${CCRAM_SIZE})
            SET(${CCRAM_SIZE} "0K")
        ENDIF()
...
FUNCTION(STM32_SET_CHIP_DEFINITIONS TARGET CHIP_TYPE)
    IF(NOT STM32_CHIP_DEFINE)
        IF(NOT DEV_${CHIP_TYPE})
            MESSAGE(FATAL_ERROR "Invalid/unsupported STM32F3 chip type: ${CHIP_TYPE}")
        ENDIF()
        GET_TARGET_PROPERTY(TARGET_DEFS ${TARGET} COMPILE_DEFINITIONS)
        LIST(GET DEV_${CHIP_TYPE} ${DEV_TABLE_DEFINE} STM32_CHIP_DEFINE)
    ENDIF()
    IF(TARGET_DEFS)
        SET(TARGET_DEFS "STM32${STM32_FAMILY};${STM32_CHIP_DEFINE};${TARGET_DEFS}")
    ELSE()
        SET(TARGET_DEFS "STM32${STM32_FAMILY};${STM32_CHIP_DEFINE}")
    ENDIF()
...
FUNCTION(STM32_SET_CHIP_DEFINITIONS TARGET)
    ...
    ENDIF()
    IF(NOT STM32_CHIP_DEFINE)
        IF(NOT DEV_${STM32_CHIP_TYPE})
            MESSAGE(FATAL_ERROR "Invalid/unsupported STM32F3 chip type: ${STM32_CHIP_TYPE}")
        ENDIF()
        GET_TARGET_PROPERTY(TARGET_DEFS ${TARGET} COMPILE_DEFINITIONS)
        LIST(GET DEV_${STM32_CHIP_TYPE} ${DEV_TABLE_DEFINE} STM32_CHIP_DEFINE)
    ENDIF()
    IF(TARGET_DEFS)
        SET(TARGET_DEFS "STM32${STM32_FAMILY};${STM32_CHIP_DEFINE};${TARGET_DEFS}")
    ELSE()
...
FUNCTION(STM32_SET_TARGET_PROPERTIES TARGET)
    ...
    IF((NOT STM32_FLASH_SIZE) OR (NOT STM32_RAM_SIZE))
        STM32_GET_CHIP_PARAMETERS(${STM32_CHIP} STM32_FLASH_ORIGIN STM32_FLASH_SIZE STM32_RAM_ORIGIN STM32_RAM_SIZE STM32_CCRAM_ORIGIN STM32_CCRAM_SIZE)
        IF((NOT STM32_FLASH_SIZE) OR (NOT STM32_RAM_SIZE))
    ...
    IF(STM32_CCRAM_SIZE STREQUAL "0K")
